...
d=d.replace(/&gt;/,">"),d=c._firefoxUrlCleanup(d);c[e.hasClass("partial")?"addPartial":"addTemplate"](e.attr("id"),d);e.remove()})};c.clearAll=function(){for(var a in c.templates)delete c[a];c.templates={};c.partials={}};c.refresh=function(){c.clearAll();c.grabTemplates()};c._firefoxUrlCleanup=function(a){return a.replace(/%7B%7B/,"{{").replace(/%7D%7D/,"}}")}};f(function(){ich.grabTemplates()})})(window.jQuery||window.Zepto);
